using System;
using System.Drawing;

using MonoTouch.ObjCRuntime;
using MonoTouch.Foundation;
using MonoTouch.UIKit;
using MonoTouch.CoreAnimation;
using MonoTouch.CoreText;

namespace JTAttributedLabel
{
	// The first step to creating a binding is to add your native library ("libNativeLibrary.a")
	// to the project by right-clicking (or Control-clicking) the folder containing this source
	// file and clicking "Add files..." and then simply select the native library (or libraries)
	// that you want to bind.
	//
	// When you do that, you'll notice that MonoDevelop generates a code-behind file for each
	// native library which will contain a [LinkWith] attribute. MonoDevelop auto-detects the
	// architectures that the native library supports and fills in that information for you,
	// however, it cannot auto-detect any Frameworks or other system libraries that the
	// native library may depend on, so you'll need to fill in that information yourself.
	//
	// Once you've done that, you're ready to move on to binding the API...
	//
	//
	// Here is where you'd define your API definition for the native Objective-C library.
	//
	// For example, to bind the following Objective-C class:
	//
	//     @interface Widget : NSObject {
	//     }
	//
	// The C# binding would look like this:
	//
	//     [BaseType (typeof (NSObject))]
	//     interface Widget {
	//     }
	//
	// To bind Objective-C properties, such as:
	//
	//     @property (nonatomic, readwrite, assign) CGPoint center;
	//
	// You would add a property definition in the C# interface like so:
	//
	//     [Export ("center")]
	//     PointF Center { get; set; }
	//
	// To bind an Objective-C method, such as:
	//
	//     -(void) doSomething:(NSObject *)object atIndex:(NSInteger)index;
	//
	// You would add a method definition to the C# interface like so:
	//
	//     [Export ("doSomething:atIndex:")]
	//     void DoSomething (NSObject object, int index);
	//
	// Objective-C "constructors" such as:
	//
	//     -(id)initWithElmo:(ElmoMuppet *)elmo;
	//
	// Can be bound as:
	//
	//     [Export ("initWithElmo:")]
	//     IntPtr Constructor (ElmoMuppet elmo);
	//
	// For more information, see http://docs.xamarin.com/ios/advanced_topics/binding_objective-c_types
	//

	[BaseType (typeof (UILabel))]
	interface JTAttributedLabel {
		[Export ("attributedText")]
		NSAttributedString JTAttributedText { get; set; }
	}

	[BaseType (typeof (CATextLayer))]
	interface JTTextLayer {
		[Export ("string", ArgumentSemantic.Copy)]
		NSObject JTString { get; set; }
	}

	[BaseType (typeof (UILabel))]
	interface JTAutoLabel {
	}

	[BaseType (typeof (NSObject))]
	interface JTParagraphStyle {
		[Export ("defaultParagraphStyle"), Static]
		JTParagraphStyle DefaultParagraphStyle ();

		[Export ("defaultWritingDirectionForLanguage:"), Static]
		NSWritingDirection DefaultWritingDirectionForLanguage (string languageName);

		[Export ("lineSpacing")]
		float LineSpacing { get; }

		[Export ("paragraphSpacing")]
		float ParagraphSpacing { get; }

		[Export ("alignment")]
		CTTextAlignment Alignment { get; }

		[Export ("headIndent")]
		float HeadIndent { get; }

		[Export ("tailIndent")]
		float TailIndent { get; }

		[Export ("firstLineHeadIndent")]
		float FirstLineHeadIndent { get; }

		[Export ("minimumLineHeight")]
		float MinimumLineHeight { get; }

		[Export ("maximumLineHeight")]
		float MaximumLineHeight { get; }

		[Export ("lineBreakMode")]
		CTLineBreakMode LineBreakMode { get; }

		[Export ("baseWritingDirection")]
		NSWritingDirection BaseWritingDirection { get; }

		[Export ("lineHeightMultiple")]
		float LineHeightMultiple { get; }

		[Export ("paragraphSpacingBefore")]
		float ParagraphSpacingBefore { get; }

		[Export ("hyphenationFactor")]
		float HyphenationFactor { get; }
	}

	[BaseType (typeof (JTParagraphStyle))]
	interface JTMutableParagraphStyle {
		[Export ("lineSpacing")]
		float LineSpacing { get; set; }

		[Export ("paragraphSpacing")]
		float ParagraphSpacing { get; set; }

		[Export ("alignment")]
		CTTextAlignment Alignment { get; set; }

		[Export ("firstLineHeadIndent")]
		float FirstLineHeadIndent { get; set; }

		[Export ("headIndent")]
		float HeadIndent { get; set; }

		[Export ("tailIndent")]
		float TailIndent { get; set; }

		[Export ("lineBreakMode")]
		CTLineBreakMode LineBreakMode { get; set; }

		[Export ("minimumLineHeight")]
		float MinimumLineHeight { get; set; }

		[Export ("maximumLineHeight")]
		float MaximumLineHeight { get; set; }

		[Export ("baseWritingDirection")]
		NSWritingDirection BaseWritingDirection { get; set; }

		[Export ("lineHeightMultiple")]
		float LineHeightMultiple { get; set; }

		[Export ("paragraphSpacingBefore")]
		float ParagraphSpacingBefore { get; set; }

		[Export ("hyphenationFactor")]
		float HyphenationFactor { get; set; }
	}
}